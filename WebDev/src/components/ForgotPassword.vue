                        ...
                        if (String(document.getElementById("password").value).length < 6) {
                            ...
                            if (document.getElementById("password").value == document.getElementById("confirmPassword").value) {
                                ...
                                if(response2.data['result'][0] == 0) {
                                    document.getElementById("error").textContent = "Email does not exist!"
                                ...
                                if(response2.data['result'][1] != response1.data['result'][1] &&
                                    response1.data['result'][0] != 0 && response2.data['result'][0] != 0) {
                                    document.getElementById("error").textContent = "Email and username don't match!"
                                    ...
                                    const response = await axios.put('http://127.0.0.1:5000/resetPassword',
                                        formData,
                                        {
                                        headers: {
                                            'Content-Type': 'multipart/form-data'
                                    ...
                                    if (response.statusText == 'OK') {
                                        ...
                                        this.$emit('userid', response2.data['result'][1])
                                    } 
                                }
